...
/// Uniform buffer layout sent to the vertex shader.
#[repr(C)]
...
impl GpuRenderer {
    ...
    /// Render graph to RGBA8 pixel buffer (row-major, top-to-bottom).
    pub fn render(
        &self,
        positions: &[[f32; 2]],
        edges: &[(usize, usize)],
        ...
        // ---- Bind group layout (vertex stage reads uniforms) ----
        ...
        // ---- Shared vertex attribute (vec2<f32> @ location 0) ----
        let vtx_attrs = [wgpu::VertexAttribute {
            format: wgpu::VertexFormat::Float32x2,
            offset: 0,
            shader_location: 0,
        }];

        // ---- Edge pipeline: LINE_LIST with alpha blending ----
        let edge_pipeline =
        ...
        // ---- Node pipeline: POINT_LIST, opaque ----
        let node_pipeline =
            self.device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
        ...
        // ---- Build edge vertex buffer (flatten pairs of positions) ----
        ...
        // bytes_per_row must be aligned to 256 for COPY_SRC→buffer
...

        {
            ...
            // Pass 2: nodes (drawn on top of edges)
        ...
        // Strip row padding to get tightly-packed RGBA8 rows
