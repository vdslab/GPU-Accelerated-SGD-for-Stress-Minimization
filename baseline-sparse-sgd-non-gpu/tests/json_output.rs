...
fn json_mode_writes_one_record_and_no_progress_to_stdout() {
    ...
    .unwrap();
    let output = Command::new(env!("CARGO_BIN_EXE_baseline-sparse-sgd-non-gpu"))
        .args([
            "--run-id",
            "contract-sparse",
            "--input",
    ...
    assert_eq!(record["method"], "sparse_sgd");
    assert_eq!(record["run_id"], "contract-sparse");
    assert_eq!(record["pivots"], 2);
