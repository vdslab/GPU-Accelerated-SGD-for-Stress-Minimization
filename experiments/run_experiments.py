...
def load_manifest(path: Path) -> dict[str, Any]:
    try:
        ...
        raise RunnerError(f"manifestを読めません: {path}: {error}") from error
    if not isinstance(value, dict):
        raise RunnerError("manifest rootはJSON objectである必要があります")
...
def validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    if missing:
        raise RunnerError(f"manifestの必須fieldがありません: {', '.join(missing)}")
    experiment_id = manifest["experiment_id"]
    if not isinstance(experiment_id, str) or not re.fullmatch(
        r"[A-Za-z0-9][A-Za-z0-9_.-]*", experiment_id
    ):
        raise RunnerError("experiment_idには英数字、-、_、.だけを使用してください")
    if manifest["run_mode"] not in {"benchmark", "diagnostic"}:
        raise RunnerError("run_modeはbenchmarkまたはdiagnosticです")
...
def expand_runs(
    ...
    if len({run.run_id for run in runs}) != len(runs):
        raise RunnerError("run IDが重複しました")
    return runs
